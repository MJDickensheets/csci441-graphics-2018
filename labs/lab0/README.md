...
Enter `qmake`, the tool we'll be using to generate our Makefiles. `qmake` comes
with Qt, a popular cross platform GUI library for C++ (and bindings for many
other languages) that we'll be using throughout the semester. `qmake` looks for
a project file (a .pro file) and generates a Makefile from the information
contained within it.
...
Now run the `qmake` command. On the Fedora lab machines, there are multiple
versions of `qmake` installed. Use `qmake-qt5` whenever I say to run `qmake`. I
recommend making an alias (ask me and I can help you set this up). Open the
`Makefile` that it generated. You'll notice that it's a whole lot more
